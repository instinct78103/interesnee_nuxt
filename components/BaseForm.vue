    ...
    <section :class="[$style.root, $style.backgrounded]" :style="{ backgroundColor: bgColor}">
      <div :class="[$style.container, $style.formContainer]">
        ...
        <form @submit.prevent="handleSubmit" style="--gap: 15px;display:flex;flex-flow:row wrap; gap: var(--gap);" autocomplete="off">
          ...
          <div :class="[$style.col, $style.col_6]">
            <input v-model="form.phone.value" @blur="validateField('phone')" :class="[$style.input, {[$style.error]: errors.phone && !isPhoneValid}]" v-mask="'+7 (###) ###-##-##'" name="phone" type="text" placeholder="Телефон" />
          ...
          <div :class="[$style.col, $style.col_12]">
            <template v-if="select">
              <select :class="[$style.input, {[$style.error]: errors.camp }, $style.select]" v-model="form.campSelect.value" @blur="validateField('campSelect')" name="campSelect">
              ...
              <input v-model="form.campSelect.value" name="camp" type="hidden" />
              <transition name="slide-top"><span v-show="errors.campSelect && !isSelectCampValid" :class="$style.detail">{{ errors.campSelect }}</span></transition>
            </template>
            <template v-else>
          ...
          <template v-if="showCampCity">
            ...
            <div v-if="form.selectedCity.value === 'other'" :class="[$style.col, $style.col_12]">
              ...
              <transition name="slide-top"><span v-show="errors.campCity && !isCampCityValid" :class="$style.detail">{{ errors.campCity }}</span></transition>
            </div>
      ...
      <dialog ref="dialogResponse">
          <div :class="$style.dialogContent">
            <h3>{{isSuccess ? 'Спасибо!' : 'Что-то пошло не так!'}}</h3>
            <p v-if="isSuccess">Мы получили ваше сообщение и свяжемся с вами в ближайшее время.</p>
            <p v-else>Не получилось обработать Ваш запрос. Вы можете связаться с нами, отправив письмо на почту <a href="mailto:hr@interesnee.ru">hr@interesnee.ru</a>, или попробовать позже.</p>
            <button form="dialogResponse">Закрыть</button>
          </div>
        <form method="dialog" id="dialogResponse"></form>
      </dialog>
...
const form = {
  ...
  userEmail: ref(''),
  campSelect: ref(''),
  vacancy: ref( jobs.value?.find(job => job.board_code === useRoute().params.code)?.title || '' ),
  selectedCity: ref(''),
  campCity: ref(''),
  link: ref(''),
  selectedStudy: ref(''),
...
const isFirstnameValid = computed(() => form.firstname.value.trim() !== '' && /^[А-Яа-я]{2,30}$/.test(form.firstname.value));
const isLastnameValid = computed(() => form.lastname.value.trim() !== '' && /^[А-Яа-я]{2,30}$/.test(form.lastname.value));
...
const isSelectCampValid = computed(() => form.campSelect.value !== '' || !props.select);
const isVacancyValid = computed(() => form.vacancy.value !== '' || props.select);
...
watch(() => form.campSelect.value, (newValue) => {
  form.vacancy.value = props.select[newValue]?.name || ''
  job.value = jobs.value.find(job => job.board_code === props.select[newValue]?.value) || ''
});
...
const validateField = (field) => {
  ...
  if (field === 'campSelect' && !isSelectCampValid.value) {
    errors.value.campSelect = 'Поле Направление практики обязательно для заполнения';
  }
  if (field === 'vacancy' && !isVacancyValid.value) {
    errors.value.vacancy = 'Поле Интересующая вакансия обязательно для заполнения';
  }
...
async function submitForm() {
  try {
    ...
    const { hash, sessionId } = await (await fetch(`${config.public.apiBaseUrl}/sign-form.php`)).json();
    ...
    const resp = await (await fetch(`${config.public.apiBaseUrl}/save-form.php`, {
      method: 'POST',
    ...
    if (isSuccess.value) {
      ...
      form.campSelect.value = '';
      form.campCity.value = '';
      form.selectedCity.value = '';
...
.dialogContent {
  ...
  width: min(480px, 100% - 40px) !important;
  margin-inline: auto;
